...
def which_shape():
    counter = 0
    print("For circle press the up botton once, for heart press it twice and thrice for square")
    while not btn.enter:
        if btn.up:
            counter += 1
            print(counter)
...
shape = which_shape()
print(shape + ' is being recorded')
filename = r'./../csv_data/shapes.csv'
f = open(filename, 'a')
...
if btn.enter:
    ...
    data_point = [lefty, midy, righty, leftm, rightm, shape]
